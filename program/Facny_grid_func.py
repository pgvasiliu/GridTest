...
def create_order_info(factor_data):
    ...
    for i in factor_data.index:
        con1 = factor_data.at[i, '上涨比例'] < 0.2
        con2 = factor_data.at[i, '上涨比例'] > 0.8
        info_dict = {
            ...
            'bench_path': candle_path + factor_data.at[i, 'symbol'] + '.%s' % file_type,
            'coin_path': candle_path + factor_data.at[i, 'symbol'] + '.%s' % file_type,
            'cap': ini_cap,
            'leverage': leverage,
            'grid_num': grid_count,
            'limit': stop_limit,
            ...
            'fee': c_rate,
            'stop_loss': stop_loss if con1 | con2 else stop_loss * 2,
