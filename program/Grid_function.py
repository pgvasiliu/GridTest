...
import numpy as np
import datetime
from Config import c_rate_taker
...
def cal_equity_curve_for_grid(candle_df, trade_df, fee, cap, margin_rate=0.05, stop_loss=0.05, stop_profit=0.02):
    ...
    df = pd.merge(left=candle_data, right=trade_data, on=['candle_begin_time'], how='outer',
                  sort=True)  # 有+15s、30s、45s，所以用outer
    ...
    # =设置止损
    df['stop_loss'] = np.nan
    df.loc[df['net_value'] < 1 - stop_loss, 'stop_loss'] = 1
    df['stop_loss'].fillna(method='ffill', inplace=True)
    df['stop_loss'].fillna(value=0, inplace=True)
...
def grid_back_test(infos):
    ...
    # =====将分钟数据转为tick数据
    tick_df = trans_candle_to_tick(df, grid_info)
    # 如果直接有tick数据的话，可以直接用，无缝衔接
